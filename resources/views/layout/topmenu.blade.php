...
@if(isset($mainmenu) && isset($menu))
    ...
    <div class="ieducar-menu-container">
        @if($topmenu && $topmenu->children && $topmenu->children->count())
                                ...
                                @if($submenu->hasLinkInSubmenu())
                                    <ul class="ieducar-sub-menu clearfix">
                                        @foreach($submenu->children->sortBy('order') as $c1)
                                            @if($c1->hasLink())
                                                <li>
                                                    <a class="@if((Request::getPathInfo() === $c1->link || in_array($c1->getKey(), $menuPaths ?? []) || (isset($currentMenu) && $c1->getKey() === $currentMenu->getKey()))) {{'ieducar-sidebar-menu-active'}} @endif"
                                                                        ...
                                                                        @if($c2->hasLinkInSubmenu())
                                                                            <ul class="ieducar-sub-menu">
                                                                                @foreach($c2->children->sortBy('order') as $c3)
                                                                                    @if($c3->isLink())
                                                                                        <li>
                                                                                            <a class="@if(isset($currentMenu) && $currentMenu->getKey() === $c3->getKey()) {{'ieducar-sidebar-menu-active'}} @endif"
