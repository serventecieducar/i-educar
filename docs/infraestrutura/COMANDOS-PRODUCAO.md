...
> **Tipo:** Referência técnica · **Módulo:** Infraestrutura / deploy · **Público:** DevOps, suporte, desenvolvimento  
> **Índice:** [Documentação](../README.md)

Referência rápida para atualizar código, dependências, caches e o módulo de relatórios. Ajuste caminhos e usuário (`www-data`, `serventec`, etc.) ao seu ambiente.

**Composer:** os exemplos usam `php composer.phar` na raiz do i-Educar (arquivo `composer.phar` junto ao `composer.json`). Se usar o binário global `composer`, substitua o prefixo.
...
| # | Seção |
|---|--------|
| — | [Pré-requisitos no servidor](#pré-requisitos-no-servidor) |
| 1 | [Atualizar o core](#1-atualizar-o-core-este-repositório) |
| 2 | [Dependências Composer](#2-dependências-composer) |
| 3 | [Pacote de relatórios](#3-pacote-de-relatórios-i-educar-reports-package) |
| 4–7 | [Laravel, FPM, permissões, fila](#4-laravel-caches-e-optimização) |
| 8–12 | [Jasper, instalações múltiplas, fork 2.11](#8-relacionado-com-jasper--relatórios) |
| — | [Ordem mínima sugerida (deploy)](#ordem-mínima-sugerida-deploy-típico) |
...
## Pacote `buriti/i-educar-setup-package` (Amélia Rodrigues e demais perfis)

Após `composer update buriti/i-educar-setup-package` e `php artisan optimize:clear`:

```bash
# Idades de referência e Educacenso no ano vigente
php artisan ieducar:idades-referencia --ano=2026 --force-production

# Atividades complementares EF — preparação (curso/regra, sem turmas)
php artisan ieducar:atividades-complementares-ef --apenas-preparacao --force-production

# Continuidade letiva (se ainda não rodou no setup)
php artisan ieducar:continuidade-letiva --ano-vigente=2026 --anos=2024,2025,2026 --force-production

# AC INEP nas turmas EF existentes
php artisan ieducar:atividades-complementares-ef --apenas-turmas --anos=2025,2026 --force-production
```

Documentação completa: `packages/buriti/i-educar-setup-package/docs/COMANDOS-ARTISAN.md`.
...
| Documento | Relação |
|-----------|---------|
| [Índice da documentação](../README.md) | Todos os guias por tema |
| [Matrícula dupla](../operacao/MATRICULA-DUPLA-REGULAR-AEE-ATIVIDADE-COMPLEMENTAR.md) | Seeds AEE, operação escolar |
| [Doc executivo — relatórios](../relatorios/DOC-EXECUTIVO-PACOTE-RELATORIOS-PORTABILIS.md) | Arquitetura Jasper |
